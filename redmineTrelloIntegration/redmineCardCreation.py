...
def create_dashboard(taskId:str,taskName:str) -> str:
    ...
    response = requests.post(URL_TRELLO.format('boards',TRELLO_API_KEY,TRELLO_TOKEN),json=jsonDatos)
...
def get_default_list_dashboard(dashboardId:str) -> str:
    ...
    response = requests.get(URL_TRELLO.format('boards/{0}/lists'.format(dashboardId),TRELLO_API_KEY,TRELLO_TOKEN))
...
def create_new_trello_card(redmineTask:RedmineTask, dashListId:str):
    ...
    cardDescription = 'HU#{0}: {1}/{0}'.format(redmineTask.id, URL_REMINE)
    ...
    jsonDatos = {"name":cardName,
            ...
            "due":endDate}
    requests.post(URL_TRELLO.format('cards',TRELLO_API_KEY,TRELLO_TOKEN),json=jsonDatos)

def get_redmine_task_info(taskId:str) -> dict:
    """ Get the Redmine Task information. """

    response = requests.get(URL_API_REMINE.format(taskId,REDMINE_API_KEY)+'&include=children')
